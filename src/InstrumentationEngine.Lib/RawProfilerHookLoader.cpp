...
namespace MicrosoftInstrumentationEngine
{
        ...
        _Out_ CModuleHandle& hRawProfilerHookModule)
    {
        ...
        C_LOGMESSAGE(_T("Looking up RawProfilerHook component module '%s'"), strModulePath);
        ...
        {
            C_LOGMESSAGE(_T("RawProfilerHook component module not found in the process, attempting to load"));

            hModule = ::LoadLibrary(strModulePath.c_str());
            IfFalseRet(nullptr != hModule, HRESULT_FROM_WIN32(::GetLastError()));

            C_LOGMESSAGE(_T("RawProfilerHook component module load complete"));
        }
        else
        {
            C_LOGMESSAGE(_T("RawProfilerHook component module already loaded"));
        }
