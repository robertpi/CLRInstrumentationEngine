    ...
    _Out_ CInstruction** ppInstruction)
{
    ...
    *ppInstruction = NULL;
    ...
    C_LOGMESSAGE(_T("Starting CInstruction::InstructionFromBytes"));
    ...
    else
    {
        ...
        else
        {
            switch (type)
            {
                ...
                default:
                {
                    C_LOGMESSAGE(_T("CInstruction::InstructionFromBytes - Incorrect operand type"));
                    return E_FAIL;
                }
            }
        }
    ...
    C_LOGMESSAGE(_T("End CInstruction::InstructionFromBytes"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CInstruction::GetOffset"));

    IfNullRetPointer(pdwOffset);

    *pdwOffset = m_offset;

    C_LOGMESSAGE(_T("End CInstruction::GetOffset"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CInstruction::GetOriginalOffset"));

    IfNullRetPointer(pdwOffset);

    *pdwOffset = m_origOffset;

    C_LOGMESSAGE(_T("End CInstruction::GetOriginalOffset"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CInstruction::GetOpCodeName"));
    ...
    *pbstrName = bstrOpCodeName.Detach();
    ...
    C_LOGMESSAGE(_T("End CInstruction::GetOpCodeName"));
 ...
 {
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CInstruction::GetOpCode"));
    IfNullRetPointer(pOpCode);

    *pOpCode = m_opcode;

    C_LOGMESSAGE(_T("End CInstruction::GetOpCode"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CInstruction::GetAlternateOrdinalOpcode"));
    IfNullRetPointer(pAlternative);

    *pAlternative = s_ilOpcodeInfo[m_opcode].m_alternate;

    C_LOGMESSAGE(_T("End CInstruction::GetAlternateOrdinalOpcode"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CInstruction::GetOpcodeLength"));
    IfNullRetPointer(pdwLength);

    *pdwLength = s_ilOpcodeInfo[m_opcode].m_opcodeLength;

    C_LOGMESSAGE(_T("End CInstruction::GetOpcodeLength"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CInstruction::GetIsRemoved"));
    IfNullRetPointer(pbValue);

    *pbValue = m_bIsRemoved;

    C_LOGMESSAGE(_T("End CInstruction::GetIsRemoved"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CInstruction::GetInstructionSize"));
    ...
    *pdwSize = dwOpCodeLength + dwOperandLength;
    ...
    C_LOGMESSAGE(_T("End CInstruction::GetInstructionSize"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CInstruction::GetIsBranch"));
    IfNullRetPointer(pbValue);

    *pbValue = IsFlagSet(s_ilOpcodeInfo[m_opcode].m_flags, ILOpcodeFlag_Branch);

    C_LOGMESSAGE(_T("End CInstruction::GetIsBranch"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CInstruction::GetIsSwitch"));
    IfNullRetPointer(pbValue);

    *pbValue = (m_opcode == Cee_Switch);

    C_LOGMESSAGE(_T("End CInstruction::GetIsSwitch"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CInstruction::GetIsCallInstruction"));
    IfNullRetPointer(pbValue);

    *pbValue = (m_opcode == Cee_Call || m_opcode == Cee_Calli || m_opcode == Cee_Callvirt);

    C_LOGMESSAGE(_T("End CInstruction::GetIsCallInstruction"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CInstruction::GetNextInstruction"));
    IfNullRetPointer(ppNextInstruction);
    *ppNextInstruction = NULL;

    if (m_pNextInstruction == NULL)
    {
        C_LOGMESSAGE(_T("CInstruction::GetNextInstruction - no next instruction"));
        return E_FAIL;
    }
    ...
    *ppNextInstruction = (IInstruction*)(m_pNextInstruction.p);
    ...
    C_LOGMESSAGE(_T("End CInstruction::GetNextInstruction"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CInstruction::GetPreviousInstruction"));
    IfNullRetPointer(ppPrevInstruction);
    *ppPrevInstruction = NULL;

    if (m_pPreviousInstruction == NULL)
    {
        C_LOGMESSAGE(_T("CInstruction::GetPreviousInstruction - no previous instruction"));
        return E_FAIL;
    }
    ...
    *ppPrevInstruction = (IInstruction*)(m_pPreviousInstruction.p);
    ...
    C_LOGMESSAGE(_T("End CInstruction::GetPreviousInstruction"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CInstruction::GetOriginalNextInstruction"));
    IfNullRetPointer(ppNextInstruction);
    *ppNextInstruction = NULL;

    if (m_pOriginalNextInstruction == NULL)
    {
        C_LOGMESSAGE(_T("CInstruction::GetOriginalNextInstruction - no original next instruction"));
        return E_FAIL;
    }
    ...
    *ppNextInstruction = (IInstruction*)(m_pOriginalNextInstruction.p);
    ...
    C_LOGMESSAGE(_T("End CInstruction::GetOriginalNextInstruction"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CInstruction::GetOriginalPreviousInstruction"));
    IfNullRetPointer(ppPrevInstruction);
    *ppPrevInstruction = NULL;

    if (m_pOriginalPreviousInstruction == NULL)
    {
        C_LOGMESSAGE(_T("CInstruction::GetOriginalPreviousInstruction - no original previous instruction"));
        return E_FAIL;
    }
    ...
    *ppPrevInstruction = (IInstruction*)(m_pOriginalPreviousInstruction.p);
    ...
    C_LOGMESSAGE(_T("End CInstruction::GetOriginalPreviousInstruction"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CInstruction::SetNextInstruction"));
    ...
    C_LOGMESSAGE(_T("End CInstruction::SetNextInstruction"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CInstruction::SetPreviousInstruction"));
    ...
    C_LOGMESSAGE(_T("End CInstruction::SetPreviousInstruction"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CInstruction::SetOriginalOffset offset is %04x"), offset);

    m_origOffset = offset;

    C_LOGMESSAGE(_T("End CInstruction::SetOriginalOffset"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CInstruction::SetOffset offset is %04x"), offset);

    m_offset = offset;

    C_LOGMESSAGE(_T("End CInstruction::SetOffset"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CInstruction::SetInstructionGeneration"));

    m_instructionGeneration = instructionGeneration;

    C_LOGMESSAGE(_T("End CInstruction::SetInstructionGeneration"));
...
HRESULT MicrosoftInstrumentationEngine::COperandInstruction::GetOperandType(_Out_ enum ILOperandType* pType)
{
    ...
    C_LOGMESSAGE(_T("Starting COperandInstruction::GetOperandType"));
    IfNullRetPointer(pType);

    *pType = s_ilOpcodeInfo[m_opcode].m_type;

    C_LOGMESSAGE(_T("End COperandInstruction::GetOperandType"));
    ...
    )
{
    ...
    C_LOGMESSAGE(_T("Starting COperandInstruction::GetOperandValue"));
    ...
    C_LOGMESSAGE(_T("End COperandInstruction::GetOperandValue"));
        ...
        )
{
    ...
    C_LOGMESSAGE(_T("Starting COperandInstruction::SetOperandValue"));
    IfNullRetPointer(pBytes);
    ...
    C_LOGMESSAGE(_T("End COperandInstruction::SetOperandValue"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CBranchInstruction::IsShortBranch"));
    IfNullRetPointer(pbValue);

    *pbValue = (m_opcode < Cee_Br || m_opcode == Cee_Leave_S);

    C_LOGMESSAGE(_T("End CBranchInstruction::IsShortBranch"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CBranchInstruction::ExpandBranch"));
    ...
    C_LOGMESSAGE(_T("End CBranchInstruction::ExpandBranch"));
...
HRESULT MicrosoftInstrumentationEngine::CBranchInstruction::GetBranchTarget(_Out_ IInstruction** ppTarget)
{
    ...
    C_LOGMESSAGE(_T("Starting CBranchInstruction::GetTargetOffset"));
    IfNullRetPointer(ppTarget);

    *ppTarget = (IInstruction*)m_pBranchTarget;
    (*ppTarget)->AddRef();

    C_LOGMESSAGE(_T("End CBranchInstruction::GetTargetOffset"));
...
HRESULT MicrosoftInstrumentationEngine::CBranchInstruction::SetBranchTarget(_In_ IInstruction* pInstruction)
{
    ...
    C_LOGMESSAGE(_T("Starting CBranchInstruction::SetBranchTarget"));
    ...
    C_LOGMESSAGE(_T("End CBranchInstruction::SetBranchTarget"));
    ...
    )
{
    ...
    C_LOGMESSAGE(_T("Starting CSwitchInstruction::Initialize"));
    ...
    C_LOGMESSAGE(_T("End CSwitchInstruction::Initialize"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CSwitchInstruction::GetBranchTarget"));
    ...
    *ppTarget = GetBranchTargetInternal(index);
    ...
    C_LOGMESSAGE(_T("End CSwitchInstruction::GetBranchTarget"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CSwitchInstruction::SetBranchTarget"));

    m_branchTargets[index] = (CInstruction*)(pTarget);

    C_LOGMESSAGE(_T("End CSwitchInstruction::SetBranchTarget"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CSwitchInstruction::RemoveBranchTargetAt"));

    m_branchTargets.erase(m_branchTargets.begin() + index);

    C_LOGMESSAGE(_T("End CSwitchInstruction::RemoveBranchTargetAt"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CSwitchInstruction::RemoveBranchTarget"));
    ...
    C_LOGMESSAGE(_T("End CSwitchInstruction::RemoveBranchTarget"));
...
{
    HRESULT hr = S_OK;
    C_LOGMESSAGE(_T("Starting CSwitchInstruction::GetBranchCount"));

    IfNullRetPointer(pBranchCount);
    *pBranchCount = (DWORD)(m_branchTargets.size());

    C_LOGMESSAGE(_T("End CSwitchInstruction::GetBranchCount"));
...
{
    C_LOGMESSAGE(_T("Starting CLoadConstInstruction::CLoadConstInstruction"));
    ...
    C_LOGMESSAGE(_T("End CLoadConstInstruction::CLoadConstInstruction"));
}
...
{
    C_LOGMESSAGE(_T("Starting CLoadLocalInstruction::CLoadLocalInstruction"));
    ...
    C_LOGMESSAGE(_T("End CLoadLocalInstruction::CLoadLocalInstruction"));
}
...
{
    C_LOGMESSAGE(_T("Starting CLoadLocalAddrInstruction::CLoadLocalAddrInstruction"));
    ...
    C_LOGMESSAGE(_T("End CLoadLocalAddrInstruction::CLoadLocalAddrInstruction"));
}
...
{
    C_LOGMESSAGE(_T("Starting CLoadLocalAddrInstruction::CStoreLocalInstruction"));
    ...
    C_LOGMESSAGE(_T("End CLoadLocalAddrInstruction::CStoreLocalInstruction"));
}
...
{
    C_LOGMESSAGE(_T("Starting CLoadLocalAddrInstruction::CLoadArgInstruction"));
    ...
    C_LOGMESSAGE(_T("End CLoadLocalAddrInstruction::CLoadArgInstruction"));
}
...
{
    C_LOGMESSAGE(_T("Starting CLoadLocalAddrInstruction::CLoadArgAddrInstruction"));
    ...
    C_LOGMESSAGE(_T("End CLoadLocalAddrInstruction::CLoadArgAddrInstruction"));
}
