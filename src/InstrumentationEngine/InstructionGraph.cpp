...
HRESULT MicrosoftInstrumentationEngine::CInstructionGraph::DecodeInstructions(_In_ LPCBYTE pCodeBase, _In_ LPCBYTE pEndOfCode)
{
    ...
    CComPtr<CInstruction> pPrevious;
    CComPtr<CInstruction> pCurrent;

    if (pEndOfCode > pCode)
    {
        IfFailRet(CInstruction::InstructionFromBytes(pCode, pEndOfCode, &pCurrent));
    }

    unordered_map<ULONG, CComPtr<CInstruction>> offsetMap;

    m_pFirstInstruction = pCurrent;
    m_pOrigFirstInstruction = m_pFirstInstruction;

    // First pass, get the opcodes
    while (pCurrent)
    {
        IfFailRet(pCurrent->SetPreviousInstruction(pPrevious, true));

        ULONG offset = (ULONG)(pCode - pCodeBase);
        offsetMap[offset] = pCurrent;
        pCurrent->SetOffset(offset);
        pCurrent->SetOriginalOffset(offset);

        // Jump to the next instruction
        DWORD instructionSize = 0;
        IfFailRet(pCurrent->GetInstructionSize(&instructionSize));
    ...
    CComPtr<CInstruction> pLast = pCurrent;
    ...
    IfFailRet(InitializeBranches(pLast, offsetMap));
...
HRESULT MicrosoftInstrumentationEngine::CInstructionGraph::InitializeBranches(_In_ CInstruction* pLastInstruction, _In_ const std::unordered_map<ULONG, CComPtr<CInstruction>>& offsetMap)
{
    HRESULT hr;
    IfNullRet(pLastInstruction);
    // Second pass, go backwards through the instructions and fix up the next pointers and finalize branch instructions
    // Note: we use raw pointers in order to reduce overhead from addref/release
    CInstruction* pCurrent = pLastInstruction;
    CInstruction* pNext = nullptr;
    while (pCurrent)
    {
        pCurrent->SetNextInstruction(pNext, true);
        ...
        if (isBranch)
        {
            CBranchInstruction* pBranch = (CBranchInstruction*)pCurrent;
            ...
            const CComPtr<CInstruction>& target = iter->second;
            pBranch->SetBranchTarget(target);
        }
        else if (isSwitch)
        {
            CSwitchInstruction* pSwitch = (CSwitchInstruction*)(pCurrent);
            ...
            for (ULONG i = 0; i < branchCount; i++)
            {
                ...
                auto iter = offsetMap.find(offset + nextOffset);
                if (iter == offsetMap.end())
                {
                ...
                const CComPtr<CInstruction>& target = iter->second;
                pSwitch->SetBranchTarget(i, target);
            }
        }
    ...
    )
{
    ...
    *pdwCorILMapmLen = 0;
    ...
    // old instrution
    CInstruction* pPrevNewInstruction;
    while (pCurrent != NULL)
    {
...
HRESULT MicrosoftInstrumentationEngine::CInstructionGraph::Replace(_In_ IInstruction* pInstructionOrig, _In_ IInstruction *pInstructionNew)
{
    ...
    pInstructionOrig->GetPreviousInstruction((IInstruction**)(&pPreviousInstruction));
    pInstructionOrig->GetNextInstruction((IInstruction**)(&pNextInstruction));
